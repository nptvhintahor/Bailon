...
class BankingVisualizer:
    def __init__(self, df_2019):
        ...
        # left=0.25 để dành chỗ cho các nhãn danh mục dài ở Slide 1
    ...
    def draw_merchant_slide(self):
        ...
        self.ax.set_title("1. TOP 15 DANH MỤC TIÊU DÙNG (2019)", fontsize=14, fontweight='bold', pad=20)
        self.ax.set_xlabel("Số lượng giao dịch")
        self.ax.set_ylabel("") # Để trống vì nhãn y đã là Category
        
        # Thêm nhãn số lượng vào cuối mỗi cột
        for i in self.ax.containers:
            self.ax.bar_label(i, padding=5, fontsize=9)

    # --- BIỂU ĐỒ 2: 5 LOẠI TÀI KHOẢN ---
    def draw_account_slide(self):
        ...
        self.ax.set_title("2. SO SÁNH 5 LOẠI TÀI KHOẢN (2019)", fontsize=14, fontweight='bold', pad=20)
    ...
    # --- BIỂU ĐỒ 3: 6 PHƯƠNG THỨC THANH TOÁN ---
    def draw_method_slide(self):
        ...
        self.ax.set_title("3. TỶ LỆ PHƯƠNG THỨC THANH TOÁN (2019)", fontsize=14, fontweight='bold', pad=20)
        self.ax.set_ylabel("Số lượng giao dịch")
...
def run_banking_analysis(file_path):
    try:
        ...
        # Lọc dữ liệu năm 2019 và bỏ các dòng lỗi ngày tháng
        df_2019 = df[df['transaction_date'].dt.year == 2019].dropna(subset=['transaction_date']).copy()
        
        if df_2019.empty:
            print("Thông báo: Không tìm thấy dữ liệu giao dịch cho năm 2019.")
            return

        BankingVisualizer(df_2019)
