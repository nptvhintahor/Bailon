...
class BankingDashboard:
    def __init__(self, file_path):
        try:
            # 1. Đọc và lọc dữ liệu năm 2019
            df = pd.read_csv(file_path)
            df['transaction_date'] = pd.to_datetime(df['transaction_date'])
            self.df_2019 = df[df['transaction_date'].dt.year == 2019].copy()
            
            if self.df_2019.empty:
                print("Không tìm thấy dữ liệu cho năm 2019!")
                return

            self.current_slide = 0
            
            # Danh sách các hàm vẽ biểu đồ (Đã xoá trang 2 cũ)
            self.slides = [
                self.draw_kyc_fraud_comparison, # Trang 1: Pie & Bar (Từ code cũ của bạn)
                self.draw_account_kyc_stats,    # Trang 2 (Mới): 5 loại tài khoản vs KYC
                self.draw_channel_kyc_stats     # Trang 3 (Mới): 6 phương thức thanh toán vs KYC
            ]
    ...
    # --- TRANG 1: TỶ LỆ KYC & TƯƠNG QUAN LỪA ĐẢO ---
    def draw_kyc_fraud_comparison(self):
        ax1 = self.fig.add_subplot(121)
        ax2 = self.fig.add_subplot(122)
        
        # Pie Chart
        kyc_counts = self.df_2019['kyc_status'].value_counts()
        ...
        ax1.set_title("1. PHÂN BỔ TRẠNG THÁI KYC (2019)", fontweight='bold', pad=20)

        # Grouped Bar
        fraud_kyc = pd.crosstab(self.df_2019['kyc_status'], self.df_2019['is_fraud'])
        fraud_kyc.columns = ['An toàn', 'Lừa đảo']
        fraud_kyc.plot(kind='bar', ax=ax2, color=['#2ECC71', '#C0392B'], edgecolor='black', alpha=0.8)
        for container in ax2.containers: ax2.bar_label(container, fmt='{:,.0f}', padding=3, fontweight='bold')
        
        ax2.set_title("2. KYC VS LỪA ĐẢO (2019)", fontweight='bold', pad=20)
        plt.suptitle("PHÂN TÍCH TƯƠNG QUAN ĐỊNH DANH (KYC) VÀ RỦI RO GIAO DỊCH NĂM 2019", fontsize=18, fontweight='bold', color='#1A5276')

    # --- TRANG 2: 5 LOẠI TÀI KHOẢN VS KYC ---
    def draw_account_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        acc_kyc = pd.crosstab(self.df_2019['account_type'], self.df_2019['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN 5 LOẠI HÌNH TÀI KHOẢN (2019)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Loại hình tài khoản")
        ax.legend(title="Trạng thái KYC")
        plt.xticks(rotation=0)

    # --- TRANG 3: 6 PHƯƠNG THỨC THANH TOÁN VS KYC ---
    def draw_channel_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        chan_kyc = pd.crosstab(self.df_2019['channel'], self.df_2019['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN 6 PHƯƠNG THỨC THANH TOÁN (2019)", fontsize=15, fontweight='bold', pad=20)
