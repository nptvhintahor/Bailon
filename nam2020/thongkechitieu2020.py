...
class MoneyVisualizer:
    ...
    # --- BIỂU ĐỒ 1: TỔNG TIỀN THEO 15 HẠNG MỤC (2020) ---
    def draw_amount_by_category(self):
        ...
        self.ax.set_title("1. TỔNG TIỀN CHI TIÊU THEO 15 HẠNG MỤC (2020)", fontsize=14, fontweight='bold')
    ...
    # --- BIỂU ĐỒ 2: TỔNG TIỀN THEO 5 LOẠI TÀI KHOẢN (2020) ---
    def draw_amount_by_account(self):
        ...
        self.ax.set_title("2. TỔNG TIỀN THEO 5 LOẠI TÀI KHOẢN (2020)", fontsize=14, fontweight='bold')
    ...
    # --- BIỂU ĐỒ 3: TỔNG TIỀN THEO 6 PHƯƠNG THỨC (2020) ---
    def draw_amount_by_method(self):
        ...
        # Vẽ biểu đồ cột đứng cho phương thức thanh toán
        sns.barplot(x='channel', y='transaction_amount', data=data, palette='coolwarm', ax=self.ax)
        self.ax.set_title("3. TỔNG TIỀN THEO 6 PHƯƠNG THỨC THANH TOÁN (2020)", fontsize=14, fontweight='bold')
...
def run_money_analysis(file_path):
    try:
        ...
        if df_2020.empty:
            print("Dữ liệu năm 2020 không tồn tại.")
            return

        MoneyVisualizer(df_2020)
