...
class StateComparisonVisualizer:
    def __init__(self, df_2020):
        self.df = df_2020
        self.states = sorted(df_2020['state'].unique())
    ...
    def draw_slide(self):
        ...
        # 1. Chuẩn bị dữ liệu ghép cho năm 2020
        ...
        # 3. Làm đẹp trục và tiêu đề (Cập nhật năm 2020)
        self.ax.set_title(f'PHÂN TÍCH DÒNG TIỀN TẠI TIỂU BANG: {state.upper()} (2020)', 
...
def run_state_analysis(file_path):
    try:
        ...
        StateComparisonVisualizer(df_2020)
