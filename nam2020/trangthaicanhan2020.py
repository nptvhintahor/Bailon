...
class BankingDashboard:
    def __init__(self, file_path):
        try:
            # 1. Đọc và lọc dữ liệu năm 2020
            df = pd.read_csv(file_path)
            df['transaction_date'] = pd.to_datetime(df['transaction_date'], errors='coerce')
            
            # CHUYỂN SANG LẤY DỮ LIỆU NĂM 2020
            self.df_2020 = df[df['transaction_date'].dt.year == 2020].copy()
            
            if self.df_2020.empty:
                print("Thông báo: Không tìm thấy dữ liệu giao dịch cho năm 2020!")
                return

            self.current_slide = 0
            
            # Danh sách các slide phân tích năm 2020
            self.slides = [
    ...
    # --- TRANG 1: TỶ LỆ KYC & TƯƠNG QUAN LỪA ĐẢO (2020) ---
    def draw_kyc_fraud_comparison(self):
        ax1 = self.fig.add_subplot(121)
        ax2 = self.fig.add_subplot(122)
        
        # Pie Chart
        kyc_counts = self.df_2020['kyc_status'].value_counts()
        ...
        ax1.set_title("1. PHÂN BỔ TRẠNG THÁI KYC (2020)", fontweight='bold', pad=20)

        # Grouped Bar
        fraud_kyc = pd.crosstab(self.df_2020['kyc_status'], self.df_2020['is_fraud'])
        ...
        ax2.set_title("2. KYC VS LỪA ĐẢO (2020)", fontweight='bold', pad=20)
        plt.suptitle("PHÂN TÍCH TƯƠNG QUAN ĐỊNH DANH (KYC) VÀ RỦI RO GIAO DỊCH NĂM 2020", 
                     fontsize=18, fontweight='bold', color='#1A5276')

    # --- TRANG 2: LOẠI TÀI KHOẢN VS KYC (2020) ---
    def draw_account_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        acc_kyc = pd.crosstab(self.df_2020['account_type'], self.df_2020['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN CÁC LOẠI HÌNH TÀI KHOẢN (2020)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Loại hình tài khoản")
        ax.set_ylabel("Số lượng giao dịch")
        ax.legend(title="Trạng thái KYC")
        plt.xticks(rotation=0)

    # --- TRANG 3: PHƯƠNG THỨC THANH TOÁN VS KYC (2020) ---
    def draw_channel_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        chan_kyc = pd.crosstab(self.df_2020['channel'], self.df_2020['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN CÁC PHƯƠNG THỨC THANH TOÁN (2020)", fontsize=15, fontweight='bold', pad=20)
