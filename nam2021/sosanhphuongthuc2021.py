...
class CategoryComparisonVisualizer:
    def __init__(self, df_2021):
        ...
        # Lấy danh sách Top 15 danh mục chi tiêu của năm 2021
    ...
    def draw_cat_vs_account(self):
        """Biểu đồ 4: Hạng mục chi tiêu vs Loại tài khoản (2021)"""
        ...
        self.ax.set_title("BIỂU ĐỒ 4: PHÂN TÍCH LOẠI TÀI KHOẢN THEO HẠNG MỤC (2021)", fontsize=14, fontweight='bold', pad=20)
    ...
    def draw_cat_vs_method(self):
        """Biểu đồ 5: Hạng mục chi tiêu vs Phương thức thanh toán (2021)"""
        ...
        self.ax.set_title("BIỂU ĐỒ 5: PHÂN TÍCH PHƯƠNG THỨC THANH TOÁN THEO HẠNG MỤC (2021)", fontsize=14, fontweight='bold', pad=20)
...
def run_analysis(file_path):
    try:
        ...
        df_2021 = df[df['transaction_date'].dt.year == 2021].dropna(subset=['transaction_date']).copy()
        
        if df_2021.empty:
            print("Thông báo: Dữ liệu năm 2021 không tồn tại hoặc bị trống.")
            return

        print(f">>> Đang xử lý phân tích hạng mục cho {len(df_2021):,} giao dịch năm 2021...")
        # Khởi chạy giao diện trực quan hóa
        CategoryComparisonVisualizer(df_2021)
