...
class MoneyVisualizer2021:
    def __init__(self, df_2021):
        ...
        # Danh sách các hàm vẽ biểu đồ dòng tiền cho năm 2021
        self.slides = [
    ...
    # --- BIỂU ĐỒ 1: TỔNG TIỀN THEO 15 HẠNG MỤC (2021) ---
    def draw_amount_by_category(self):
        ...
        self.ax.set_title("1. TỔNG TIỀN CHI TIÊU THEO 15 HẠNG MỤC (2021)", fontsize=14, fontweight='bold')
    ...
    # --- BIỂU ĐỒ 2: TỔNG TIỀN THEO 5 LOẠI TÀI KHOẢN (2021) ---
    def draw_amount_by_account(self):
        ...
        self.ax.set_title("2. TỔNG TIỀN THEO 5 LOẠI TÀI KHOẢN (2021)", fontsize=14, fontweight='bold')
    ...
    # --- BIỂU ĐỒ 3: TỔNG TIỀN THEO 6 PHƯƠNG THỨC (2021) ---
    def draw_amount_by_method(self):
        ...
        self.ax.set_title("3. TỔNG TIỀN THEO 6 PHƯƠNG THỨC THANH TOÁN (2021)", fontsize=14, fontweight='bold')
...
def run_money_analysis(file_path):
    try:
        ...
        # CHUYỂN SANG LẤY DỮ LIỆU NĂM 2021
        df_2021 = df[df['transaction_date'].dt.year == 2021].copy()
        
        if df_2021.empty:
            print("Thông báo: Dữ liệu năm 2021 không tồn tại.")
            return

        print(f">>> Đang phân tích dòng tiền cho {len(df_2021):,} giao dịch năm 2021...")
        MoneyVisualizer2021(df_2021)
