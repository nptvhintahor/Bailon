...
class TimeAnalysisVisualizer:
    def __init__(self, df_2021):
        ...
        # Danh sách slide phân tích thời gian năm 2021
        self.slides = [
    ...
    # --- BIỂU ĐỒ 1: TỔNG QUAN GIỜ GIAO DỊCH (2021) ---
    def draw_hourly_distribution(self):
        ...
        self.ax.set_title("1. PHÂN BỔ SỐ LƯỢNG GIAO DỊCH THEO KHUNG GIỜ (2021)", fontsize=15, fontweight='bold', pad=20)
        self.ax.set_xlabel("Giờ trong ngày (0h - 23h)", fontsize=12)
        self.ax.set_ylabel("Số lượng giao dịch", fontsize=12)
        
        # Ghi chú giờ có giao dịch nhiều nhất năm 2021
    ...
    # --- BIỂU ĐỒ 2: HEATMAP THÁNG VS GIỜ (2021) ---
    def draw_monthly_hourly_heatmap(self):
        self.df['Month'] = self.df['transaction_date'].dt.month
        
        # Tạo bảng chéo giữa Tháng và Giờ cho dữ liệu 2021
        ...
        self.ax.set_title("2. BIỂU ĐỒ TƯƠNG QUAN GIỮA THÁNG VÀ GIỜ GIAO DỊCH (2021)", fontsize=15, fontweight='bold', pad=20)
        self.ax.set_xlabel("Giờ trong ngày", fontsize=12)
        self.ax.set_ylabel("Tháng trong năm", fontsize=12)
        
        # Đảm bảo các nhãn tháng hiển thị đúng theo dữ liệu thực tế có trong năm 2021
...
def run_analysis(file_path):
    try:
        ...
        print(f">>> Đang phân tích thời điểm giao dịch cho {len(df_2021):,} dữ liệu năm 2021...")
        TimeAnalysisVisualizer(df_2021)
