...
class StateComparisonVisualizer:
    def __init__(self, df_2021):
        self.df = df_2021
        self.states = sorted(df_2021['state'].unique())
    ...
    def draw_slide(self):
        ...
        # 3. Làm đẹp trục và tiêu đề (Cập nhật năm 2021)
        self.ax.set_title(f'PHÂN TÍCH DÒNG TIỀN TẠI TIỂU BANG: {state.upper()} (2021)', 
...
def run_state_analysis(file_path):
    try:
        ...
        print(f">>> Đang khởi chạy phân tích chi tiết cho {len(df_2021):,} giao dịch tại các tiểu bang năm 2021...")
        StateComparisonVisualizer(df_2021)
