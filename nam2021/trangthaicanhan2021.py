...
class BankingDashboard2021:
    def __init__(self, file_path):
        try:
            # 1. Đọc và lọc dữ liệu năm 2021
            df = pd.read_csv(file_path)
            df['transaction_date'] = pd.to_datetime(df['transaction_date'], errors='coerce')
            
            # LỌC DỮ LIỆU NĂM 2021
            self.df_2021 = df[df['transaction_date'].dt.year == 2021].copy()
            
            if self.df_2021.empty:
                print("Thông báo: Không tìm thấy dữ liệu giao dịch cho năm 2021!")
                return

            self.current_slide = 0
            
            # Danh sách các slide phân tích năm 2021
            self.slides = [
    ...
    # --- TRANG 1: TỶ LỆ KYC & TƯƠNG QUAN LỪA ĐẢO (2021) ---
    def draw_kyc_fraud_comparison(self):
        ax1 = self.fig.add_subplot(121)
        ax2 = self.fig.add_subplot(122)
        
        # Biểu đồ tròn
        kyc_counts = self.df_2021['kyc_status'].value_counts()
        ...
        ax1.set_title("1. PHÂN BỔ TRẠNG THÁI KYC (2021)", fontweight='bold', pad=20)

        # Biểu đồ cột nhóm
        fraud_kyc = pd.crosstab(self.df_2021['kyc_status'], self.df_2021['is_fraud'])
        # Đảm bảo có đủ 2 cột ngay cả khi không có dữ liệu lừa đảo ở một trạng thái nào đó
        ...
        ax2.set_title("2. KYC VS LỪA ĐẢO (2021)", fontweight='bold', pad=20)
        plt.suptitle("PHÂN TÍCH TƯƠNG QUAN ĐỊNH DANH (KYC) VÀ RỦI RO GIAO DỊCH NĂM 2021", 
                     fontsize=18, fontweight='bold', color='#1A5276')

    # --- TRANG 2: LOẠI TÀI KHOẢN VS KYC (2021) ---
    def draw_account_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        acc_kyc = pd.crosstab(self.df_2021['account_type'], self.df_2021['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN CÁC LOẠI HÌNH TÀI KHOẢN (2021)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Loại hình tài khoản")
        ax.set_ylabel("Số lượng giao dịch")
        ax.legend(title="Trạng thái KYC")
        plt.xticks(rotation=0)

    # --- TRANG 3: PHƯƠNG THỨC THANH TOÁN VS KYC (2021) ---
    def draw_channel_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        chan_kyc = pd.crosstab(self.df_2021['channel'], self.df_2021['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN CÁC PHƯƠNG THỨC THANH TOÁN (2021)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Số lượng giao dịch")
        ax.set_ylabel("Kênh giao dịch (Channel)")
        ax.legend(title="Trạng thái KYC")

if __name__ == "__main__":
    BankingDashboard2021('indian_banking_transactions.csv')