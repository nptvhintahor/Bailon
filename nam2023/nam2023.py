...
def analyze_full_report_2023(file_path):
    try:
        ...
        # 2. Lọc dữ liệu duy nhất trong năm 2023
        df_2023 = df[df['transaction_date'].dt.year == 2023].copy()
        total_tx_2023 = len(df_2023)

        if df_2023.empty:
            print("Không có dữ liệu giao dịch trong năm 2023.")
            return

        # 3. Phân tích Khách hàng (Làm sạch trùng lặp)
        unique_customers_count = df_2023['customer_id'].nunique()
        duplicate_customer_tx = df_2023.duplicated(subset=['customer_id']).sum()

        # 4. Phân tích Merchant Category
        unique_categories = df_2023['merchant_category'].nunique()
        category_counts = df_2023['merchant_category'].value_counts()
        duplicate_categories = category_counts[category_counts > 1]

        # 5. Phân tích Account Type
        acc_counts = df_2023['account_type'].value_counts()
        unique_acc_types = len(acc_counts)

        # 6. Phân tích Payment Method (Sử dụng cột 'channel')
        method_col = 'channel'
        method_counts = df_2023[method_col].value_counts()
        ...
        combined_stats = df_2023.groupby(['account_type', method_col]).size().reset_index(name='count')
        combined_duplicates = combined_stats[combined_stats['count'] > 1].sort_values(by='count', ascending=False)

        # --- XUẤT BÁO CÁO TỔNG HỢP ---
        print("="*80)
        print(f"{'BÁO CÁO TỔNG LỰC DỮ LIỆU NGÂN HÀNG NĂM 2023':^80}")
        print("="*80)

        # Phần I: Giao dịch & Khách hàng
        print(f"I. GIAO DỊCH & KHÁCH HÀNG")
        print(f"- Tổng số lượt giao dịch trong năm:         {total_tx_2023}")
...
if __name__ == "__main__":
    analyze_full_report_2023('indian_banking_transactions.csv')