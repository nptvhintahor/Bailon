...
class StateComparisonVisualizer:
    def __init__(self, df_2023):
        self.df = df_2023
        self.states = sorted(df_2023['state'].unique())
    ...
    def draw_slide(self):
        ...
        # 1. Chuẩn bị dữ liệu ghép cho năm 2023
        ...
        # 3. Làm đẹp trục và tiêu đề (Cập nhật năm 2023)
        self.ax.set_title(f'PHÂN TÍCH DÒNG TIỀN TẠI TIỂU BANG: {state.upper()} (2023)', 
...
def run_state_analysis(file_path):
    try:
        ...
        StateComparisonVisualizer(df_2023)
