...
class TimeAnalysisVisualizer:
    def __init__(self, df_2024):
        ...
        # Danh sách slide phân tích thời gian năm 2024
        self.slides = [
    ...
    # --- BIỂU ĐỒ 1: TỔNG QUAN GIỜ GIAO DỊCH TRONG NĂM 2024 ---
    def draw_hourly_distribution(self):
        # Đảm bảo có cột transaction_hour
    ...
    def draw_monthly_hourly_heatmap(self):
        ...
        self.ax.set_title("2. BIỂU ĐỒ TƯƠNG QUAN THÁNG VÀ GIỜ GIAO DỊCH (2024)", fontsize=15, fontweight='bold', pad=20)
        self.ax.set_xlabel("Giờ trong ngày", fontsize=12)
        self.ax.set_ylabel("Tháng trong năm", fontsize=12)
        
        # Cập nhật nhãn tháng dựa trên dữ liệu thực tế có trong năm 2024
        existing_months = sorted(self.df['Month'].unique())
        self.ax.set_yticklabels([f"Tháng {int(i)}" for i in existing_months], rotation=0)
...
def run_analysis(file_path):
    try:
        ...
        df_2024 = df[df['transaction_date'].dt.year == 2024].dropna(subset=['transaction_date']).copy()
        
        if df_2024.empty:
            print("Thông báo: Không tìm thấy dữ liệu giao dịch cho năm 2024.")
            return

        TimeAnalysisVisualizer(df_2024)
