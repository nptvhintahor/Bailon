...
class StateComparisonVisualizer:
    def __init__(self, df_2024):
        self.df = df_2024
        # Lấy danh sách các tiểu bang có giao dịch trong năm 2024
        self.states = sorted(df_2024['state'].dropna().unique())
    ...
    def draw_slide(self):
        ...
        # 3. Làm đẹp trục và tiêu đề cho năm 2024
        self.ax.set_title(f'PHÂN TÍCH DÒNG TIỀN 2024 - TIỂU BANG: {state.upper()}', 
...
def run_state_analysis(file_path):
    try:
        ...
        StateComparisonVisualizer(df_2024)
    except Exception as e:
        print(f"Lỗi: {e}")

if __name__ == "__main__":
    # Đảm bảo file CSV nằm cùng thư mục hoặc thay đổi đường dẫn phù hợp
