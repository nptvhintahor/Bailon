...
class BankingDashboard:
    def __init__(self, file_path):
        try:
            # 1. Đọc và lọc dữ liệu năm 2024
            df = pd.read_csv(file_path)
            df['transaction_date'] = pd.to_datetime(df['transaction_date'], errors='coerce')
            
            # Lọc dữ liệu chính xác cho năm 2024
            self.df_2024 = df[df['transaction_date'].dt.year == 2024].copy()
            
            if self.df_2024.empty:
                print("Thông báo: Không tìm thấy dữ liệu giao dịch cho năm 2024!")
                return

            self.current_slide = 0
            
            # Danh sách các hàm vẽ biểu đồ phân tích Dashboard 2024
            self.slides = [
                self.draw_kyc_fraud_comparison, # Trang 1: Pie & Bar (KYC vs Fraud)
    ...
    # --- TRANG 1: TỶ LỆ KYC & TƯƠNG QUAN LỪA ĐẢO (2024) ---
    def draw_kyc_fraud_comparison(self):
        ax1 = self.fig.add_subplot(121)
        ax2 = self.fig.add_subplot(122)
        
        # Pie Chart: Trạng thái KYC 2024
        kyc_counts = self.df_2024['kyc_status'].value_counts()
        ...
        ax1.set_title("1. PHÂN BỔ TRẠNG THÁI KYC (2024)", fontweight='bold', pad=20)

        # Grouped Bar: KYC vs Lừa đảo 2024
        fraud_kyc = pd.crosstab(self.df_2024['kyc_status'], self.df_2024['is_fraud'])
        # Đảm bảo các cột tồn tại để tránh lỗi hiển thị
        ...
        ax2.set_title("2. KYC VS LỪA ĐẢO (2024)", fontweight='bold', pad=20)
        plt.suptitle("PHÂN TÍCH TƯƠNG QUAN ĐỊNH DANH (KYC) VÀ RỦI RO GIAO DỊCH NĂM 2024", 
                     fontsize=18, fontweight='bold', color='#1A5276')

    # --- TRANG 2: LOẠI TÀI KHOẢN VS KYC (2024) ---
    def draw_account_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        acc_kyc = pd.crosstab(self.df_2024['account_type'], self.df_2024['kyc_status'])
        
        # Lọc các cột hiện có trong dữ liệu thực tế
        ...
        ax.set_title("THỐNG KÊ KYC TRÊN CÁC LOẠI HÌNH TÀI KHOẢN (2024)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Loại hình tài khoản", fontweight='bold')
        ax.set_ylabel("Số lượng khách hàng", fontweight='bold')
        ax.legend(title="Trạng thái KYC")
        plt.xticks(rotation=15)

    # --- TRANG 3: PHƯƠNG THỨC THANH TOÁN VS KYC (2024) ---
    def draw_channel_kyc_stats(self):
        ax = self.fig.add_subplot(111)
        chan_kyc = pd.crosstab(self.df_2024['channel'], self.df_2024['kyc_status'])
        ...
        ax.set_title("THỐNG KÊ KYC THEO PHƯƠNG THỨC THANH TOÁN (2024)", fontsize=15, fontweight='bold', pad=20)
        ax.set_xlabel("Số lượng giao dịch", fontweight='bold')
        ax.set_ylabel("Kênh giao dịch (Channel)", fontweight='bold')
        ax.legend(title="Trạng thái KYC")
